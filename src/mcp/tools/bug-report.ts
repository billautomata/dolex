...
import type { OperationEntry } from './operation-log.js';
...
export function anonymizeColumns(
...
export function sanitizeSpecConfig(config: Record<string, unknown>): Record<string, string> {
...
function buildOperationsTable(entries: OperationEntry[]): string {
  if (entries.length === 0) return '_No recent operations._';
...
export function handleReportBug(deps: ReportDeps) {
  return async (args: z.infer<typeof bugReportInputSchema>) => {
    ...
    if (args.specId) {
      ...
      if (stored) {
        const spec = stored.spec as any;
        const columns = anonymizeColumns(
          stored.columns.map(c => ({ name: c.name, type: c.type })),
          includeFieldNames,
        );
        const contextLines = [
        ...
        if (spec.encoding) {
          ...
          const encodingFields: string[] = [];
          for (const [channel, enc] of Object.entries(spec.encoding)) {
            const field = (enc as any)?.field;
            if (field) {
