...
import { isDashboardSpec } from '../src/types.js';
import { buildDashboardHtml } from '../src/renderers/html/builders/dashboard.js';
// ─── TEST DATA ──────────────────────────────────────────────────────────────
const sampleData = [
...
function makeViewSpec(pattern, title) {
    return {
        pattern,
        title,
        data: sampleData,
...
function makeDashboardSpec(overrides) {
    return {
        dashboard: true,
        id: 'test-dashboard',
...
describe('isDashboardSpec', () => {
    ...
    it('returns false for an atomic visualization spec', () => {
        expect(isDashboardSpec(makeViewSpec('bar', 'Test'))).toBe(false);
    });
    it('returns false for a compound spec', () => {
        ...
        expect(isDashboardSpec(compound)).toBe(false);
    });
});
...
describe('Dashboard type construction', () => {
    ...
    it('views have per-view queries', () => {
        const spec = makeDashboardSpec();
        const view1 = spec.views[0];
        const view2 = spec.views[1];
