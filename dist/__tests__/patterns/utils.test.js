...
import { parseIntent, buildMatchContext, hasTimeSeriesColumn, hasNegativeValues, } from '../../src/patterns/utils.js';
// ── Helpers ──────────────────────────────────────────────────────────────────
function col(name, type, uniqueCount = 10) {
    return {
        name,
        type,
        sampleValues: [],
...
describe('hasNegativeValues', () => {
    ...
    it('should return false when there are no numeric columns', () => {
        ...
        const columns = [col('name', 'categorical')];
        expect(hasNegativeValues(data, columns)).toBe(false);
    });
    it('should return false for empty data', () => {
        const columns = [col('value', 'numeric')];
