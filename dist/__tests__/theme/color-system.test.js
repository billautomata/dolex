...
const mockD3Scale = {
    scaleOrdinal: () => ({
        domain: (d) => mockD3Scale.scaleOrdinal(),
        range: (r) => ({ _domain: null, _range: r }),
    }),
    scaleLinear: () => ({
        domain: (d) => mockD3Scale.scaleLinear(),
        range: (r) => ({ _domain: [], _range: r }),
        interpolate: (i) => ({ _domain: null, _range: null }),
    }),
    ...
    scaleSequential: (interpolator) => ({
        domain: (d) => ({ _interpolator: interpolator }),
    }),
    interpolateViridis: (t) => '#440154',
};
// Simplified buildColorScale for testing the logic
function buildColorScale(encoding, data) {
...
//# sourceMappingURL=color-system.test.js.map