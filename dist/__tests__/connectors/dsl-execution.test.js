...
describe('SourceManager DSL Query Execution', () => {
    ...
    it('aggregation with groupBy', async () => {
        ...
        const topTotal = result.rows[0].total_revenue;
        expect(topTotal).toBeGreaterThanOrEqual(result.rows[1].total_revenue);
        await manager.closeAll();
        fs.rmSync(tmpDir, { recursive: true });
    });
...
//# sourceMappingURL=dsl-execution.test.js.map