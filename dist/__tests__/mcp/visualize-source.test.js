...
describe('visualize_from_source', () => {
    ...
    it('handler returns error when source manager not available', async () => {
        ...
        const handler = handleVisualizeFromSource(() => ({ recommended: { pattern: 'bar', spec: {}, reasoning: '' }, alternatives: [] }), { sourceManager: null });
        const result = await handler({
            sourceId: 'src-abc',
            table: 'sales',
            query: { select: ['x'] },
    ...
    it('handler returns error on query failure', async () => {
        ...
        const handler = handleVisualizeFromSource(() => ({ recommended: { pattern: 'bar', spec: {}, reasoning: '' }, alternatives: [] }), { sourceManager: mockSourceManager });
        const result = await handler({
            sourceId: 'src-abc',
            table: 'nonexistent',
            query: { select: ['x'] },
    ...
    it('handler returns visualization on successful query', async () => {
        ...
        const handler = handleVisualizeFromSource((input) => ({
            recommended: {
                pattern: 'bar',
                spec: {
                    pattern: 'bar',
...
//# sourceMappingURL=visualize-source.test.js.map