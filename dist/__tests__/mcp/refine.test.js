...
// ─── HELPERS ─────────────────────────────────────────────────────────────────
function makeSpec(pattern, overrides) {
    return {
        pattern,
        title: `Test ${pattern}`,
        data: [
...
function makeCompoundSpec(overrides) {
    return {
        compound: true,
        title: 'Compound Test',
        data: [
...
describe('refine_visualization', () => {
    ...
    describe('atomic refinements', () => {
        it('sort descending', async () => {
            ...
            expect(stored.spec.encoding.x?.sort).toBe('descending');
            expect(stored.spec.encoding.y?.sort).toBe('descending');
        });
        it('sort ascending', async () => {
            ...
            const stored = specStore.get(parsed.specId);
            expect(stored.spec.encoding.x?.sort).toBe('ascending');
            expect(stored.spec.encoding.y?.sort).toBe('ascending');
        });
        it('limit / top N', async () => {
            ...
            const stored = specStore.get(parsed.specId);
            expect(stored.spec.data).toHaveLength(2);
        });
        ...
        it('title change', async () => {
            ...
            const stored = specStore.get(parsed.specId);
            expect(stored.spec.title).toBe('Revenue Overview');
        });
        ...
        it('highlight with color field — no matching values is silently skipped', async () => {
            ...
            expect(parsed.changes.some((c) => c.includes('Highlighted'))).toBe(false);
        });
        ...
        it('colorBy refinement', async () => {
            ...
            const stored = specStore.get(parsed.specId);
            expect(stored.spec.config.colorBy).toBe('source');
        });
        ...
        it('unrecognized refinement', async () => {
            ...
            const stored = specStore.get(parsed.specId);
            expect(stored.spec.config._pendingRefinement).toBe('make it sparkle');
        });
