...
describe('SpecStore', () => {
    ...
    it('get retrieves a stored spec', () => {
        ...
        expect(stored.spec.pattern).toBe('bar');
    });
    ...
    it('updateSpec creates a new ID and stores new spec', () => {
        ...
        expect(store.get(id2).spec.pattern).toBe('line');
    });
    it('updateSpec preserves columns and alternatives from original', () => {
        const cols = [{ name: 'x', type: 'numeric', sampleValues: [], uniqueCount: 1, nullCount: 0, totalCount: 1 }];
        const alts = new Map();
...
//# sourceMappingURL=spec-store.test.js.map