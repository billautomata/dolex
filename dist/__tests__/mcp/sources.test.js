...
import { handleListSources, handleAddSource, handleRemoveSource, handleDescribeSource, isSandboxPath, } from '../../src/mcp/tools/sources.js';
/**
...
describe('handleAddSource', () => {
    it('reconnects when source already exists by name', async () => {
        ...
        const result = await handler({
            name: 'mydata',
            type: 'csv',
            config: { type: 'csv', path: '/tmp/data.csv' },
            detail: 'full',
        });
    ...
    it('returns error when sourceManager.add() throws', async () => {
        ...
        const result = await handler({
            name: 'baddb',
            type: 'postgres',
            config: { type: 'postgres', host: 'localhost', port: 5432, database: 'nope', user: 'u', password: 'p' },
            detail: 'full',
        });
    ...
    it('returns ENOENT guidance when file not found', async () => {
        ...
        const result = await handler({
            name: 'missing',
            type: 'csv',
            config: { type: 'csv', path: '/Users/bill/Downloads/nonexistent.csv' },
            detail: 'full',
        });
        expect(result.isError).toBe(true);
        const body = JSON.parse(result.content[0].text);
    ...
    it('returns error when sourceManager.add() returns ok: false', async () => {
        ...
        const result = await handler({
            name: 'dup',
            type: 'csv',
            config: { type: 'csv', path: '/tmp/dup.csv' },
            detail: 'full',
        });
        expect(result.isError).toBe(true);
        const body = JSON.parse(result.content[0].text);
...
describe('handleAddSource sandbox path rejection', () => {
    it('rejects /mnt/user-data/uploads/ paths with guidance', async () => {
        ...
        const result = await handler({
            name: 'test',
            type: 'csv',
            config: { type: 'csv', path: '/mnt/user-data/uploads/data.csv' },
            detail: 'full',
        });
        expect(result.isError).toBe(true);
        const body = JSON.parse(result.content[0].text);
    ...
    it('rejects /home/claude/ paths with guidance', async () => {
        ...
        const result = await handler({
            name: 'test',
            type: 'csv',
            config: { type: 'csv', path: '/home/claude/data.csv' },
            detail: 'full',
        });
        expect(result.isError).toBe(true);
        const body = JSON.parse(result.content[0].text);
    ...
    it('rejects sandbox paths for sqlite too', async () => {
        ...
        const result = await handler({
            name: 'test',
            type: 'sqlite',
            config: { type: 'sqlite', path: '/mnt/user-data/uploads/db.sqlite' },
            detail: 'full',
        });
        expect(result.isError).toBe(true);
        const body = JSON.parse(result.content[0].text);
    ...
    it('allows normal local paths through', async () => {
        ...
        const result = await handler({
            name: 'test',
            type: 'csv',
            config: { type: 'csv', path: '/Users/bill/Downloads/data.csv' },
            detail: 'full',
        });
        expect(result.isError).toBeUndefined();
    });
